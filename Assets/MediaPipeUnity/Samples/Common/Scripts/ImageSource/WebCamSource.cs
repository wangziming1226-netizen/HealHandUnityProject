...
#pragma warning restore IDE0025
    ...
    private IEnumerator Initialize()
    {
      ...
      {
        Debug.LogWarning($"{_TAG}: No camera devices found");
        yield break;
      }
      ...
      foreach (var dev in availableSources)
      {
      ...
      if (chosen == null)
      {
        chosen = availableSources[0];
      }

      webCamDevice = chosen;
      Debug.Log($"{_TAG}: Selected camera = {((WebCamDevice)chosen).name}, front = {((WebCamDevice)chosen).isFrontFacing}");
    }
